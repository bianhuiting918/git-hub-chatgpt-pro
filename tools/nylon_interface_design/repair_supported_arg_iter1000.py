"""Single-variable diagnostic: 1000 vs 500 iterations; all scientific gates unchanged."""
import os
...
for k,(aid,q) in enumerate(zip(heavy,start)):
 ...
 pose.add_constraint(R.core.scoring.constraints.CoordinateConstraint(aid,anchor,R.numeric.xyzVector_double_t(*map(float,q)),R.core.scoring.func.HarmonicFunc(0,.5)))
 w=world(q[None,:])[0]
...
report=dict(status='RUNNING_DIAGNOSTIC',before=audit(),material_site_index=257,material_site=site['site_id'],material_constraints=nconstraints,max_iterations=1000,restraint_sd_A=.5,exclusion_sd_A=.2,material_neighbor_radius_A=8,fixed_heavy_atoms=len(fixed),limitations=['Placeholder sequence, not complete designed protein','Fixed motif heavy atoms exact; hydrogens allowed to optimize','Soft material constraints require independent final replay','No folding or catalytic PASS'])
(out/'before.json').write_text(json.dumps(report,indent=2));pose.dump_pdb(str(out/'before.pdb'));print('START',flush=True)
mini=R.protocols.minimization_packing.MinMover();mini.movemap(mm);mini.score_function(sf);mini.min_type('lbfgs_armijo_nonmonotone');mini.tolerance(1e-6);mini.max_iter(1000);mini.cartesian(True);mini.apply(pose)
