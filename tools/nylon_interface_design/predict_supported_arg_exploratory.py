...
src=outroot/'supported_ARG_SC4559268_mpnn20_v1/seqs/supported_ARG_SC4559268_mpnn20_v1.fa'
records=[]
...
assert all(len(s)==126 and ''.join(s[i-1] for i in fixed)=='TDGDGGRGG' for s in seqs)
queries={f'R6_SUP_ARG_{i:02d}':s for i,s in enumerate(seqs,1)}
g=r/'route6_supported_arg_af20_v1';g.mkdir(exist_ok=False)
...
msadir=outroot/'route6_supported_arg_msa20_v1';afdir=outroot/'af2_route6_supported_arg20_v1';inp=g/'msa_inputs';inp.mkdir()
...
def log(stage,**kw):
 rec=dict(time=datetime.datetime.now(datetime.timezone.utc).isoformat(),step='supported_arg_af20_'+stage,**kw)
