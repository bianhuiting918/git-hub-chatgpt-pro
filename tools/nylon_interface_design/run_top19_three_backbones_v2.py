"""Three first backbones per distinct donor candidate, sequential A100 via gpurun.
...
o=base/'first19_RFD3_v2';o.mkdir(exist_ok=False)
shape=base/'candidate_specific_shapes_v2'
def log(stage,**kw):
...
for row in good:
 ...
 while not mf.exists():
  p=Path('/proc/1469075/cmdline')
  if not p.exists() or b'prepare_top50_fragment_shapes_v2.py' not in p.read_bytes():raise RuntimeError('Preparation stopped before input '+name)
 ...
 cmd=[a.replace('inference.num_designs=1','inference.num_designs=3').replace('route6_top50_local_shapes_v1','route6_top50_local_shapes_3seeds_v2') for a in j['command']];assert cmd[0]=='/usr/local/bin/gpurun' and 'CUDA_VISIBLE_DEVICES=1' in cmd
 assert any('ActiveSite_ckpt.pt' in x for x in cmd)
 out=Path(j['output'].replace('route6_top50_local_shapes_v1','route6_top50_local_shapes_3seeds_v2'));out.mkdir(parents=True,exist_ok=False,mode=0o2770);out.chmod(0o2770)
 assert not list(out.iterdir())
 with (o/(name+'.stdout')).open('x') as f:
  p=subprocess.Popen(cmd,cwd='/data/bht2/shape_conditioned_rfdiffusion_20260905_v1/source/RFdiffusion',stdout=f,stderr=subprocess.STDOUT)
 ...
 for seed in range(3):
  pdb=out/f'design_{seed}.pdb'
  assert pdb.exists() and pdb.stat().st_size>1000
  completed.append(dict(name=name,seed=seed,path=str(pdb),sha256=hashlib.sha256(pdb.read_bytes()).hexdigest()))
  log('candidate_rfd_technical_complete_NOT_GEOMETRY_PASS',**completed[-1])
with (o/'completion.json').open('x') as f:json.dump(dict(status='RFD_TECHNICAL_COMPLETE_REQUIRES_GEOMETRY_AUDIT',expected=57,completed=completed),f,indent=2)
log('first19_batch_finished',completed=len(completed),expected=57)
