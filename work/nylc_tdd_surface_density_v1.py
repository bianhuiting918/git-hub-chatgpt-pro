...
def run(mat,dp,out):
 ...
 for source in audit['sites']:
  ...
  for side,(interp,sign) in side_data.items():
   h=float(interp(q[:2][None])[0]); offsets[side]=sign*(q[2]-h) if np.isfinite(h) else None
...
if __name__=='__main__':
 out=ROOT/'surface_density_v1';out.mkdir(exist_ok=False)
 status='FAILED';rc=1
 try:
