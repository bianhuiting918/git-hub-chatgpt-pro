...
def process_mesh(mesh,halo,xyz,cat,segs,sites,box,out,save):
    ...
    keep=(area>1e-10)&np.all((cent[:,:2]>=0)&(cent[:,:2]<box[:2]),axis=1)
    ...
    summaries={};rows_by_side={};atom_areas={}
    ...
    if save:
        ...
        for site in sites:
            ...
            for side in ["top","bottom"]:
                ...
                row[side+"_status"]="SURFACE_ASSIGNED" if ar>0 else "NOT_ON_RENDERED_SURFACE"
            allrows.append(row)
        table(out/"all_8000_esters.csv",allrows)
        plot_results(out,rows_by_side,box)
    summaries["z_midplane_A"]=mid
    summaries["surface_definition"]="PyMOL SES triangles with centroid in central periodic xy cell; top/bottom split at polymer median z. All chain ends retained."
...
def run_analysis(args):
    ...
    try:
        ...
        for q in [1,2]:
            mesh=out/f"pymol_surface_q{q}.npz"
            command=[str(args.pymol_python),"-B",str(Path(__file__).resolve()),"--export",str(out/"mesh_input.npz"),str(mesh),str(q)]
            record("EXPORT_START",json.dumps(command))
            with (out/f"pymol_q{q}.log").open("w") as f:
                result=subprocess.run(command,stdout=f,stderr=subprocess.STDOUT,env={**os.environ,"OMP_NUM_THREADS":"8","OPENBLAS_NUM_THREADS":"1"})
            record("EXPORT_EXIT",str(result.returncode))
            if result.returncode:raise RuntimeError("PyMOL export failed; inspect "+str(out/f"pymol_q{q}.log"))
            print("MESH_EXPORTED",q,flush=True)
        ...
        audit2={"technical_status":"COMPLETE","numerical_status":{s:convergence[s]["numerical_gate"] for s in convergence},
            ...
            "top_bottom":"Central-cell mesh face centroids split at median polymer z. No free-chain or tail removal. Rendered surface definition, not vertical-only envelope.",
        ...
        print("FINAL",json.dumps({"output":str(out),"numerical":audit2["numerical_status"],"convergence":convergence}),flush=True)
    except Exception:
...
def main():
    ...
    a=ap.parse_args()
    if a.selftest_pymol:export_mesh(None,None,2,True)
