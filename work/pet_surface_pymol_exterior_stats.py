...
def exterior_mask(v,faces):
    ...
    shells=[];retained=[];excluded=[];checks=[]
    def make_shell(i):
    ...
    def inside(shell,p):
        ...
        assert len(set(parities))==1,("Ray containment ambiguous",shell["id"],p.tolist(),parities)
    ...
    audit={"components":nc,"external_components":retained,"excluded_nested_components":excluded,
        ...
        "kept_mesh_area_A2":float(area[keep].sum()),"excluded_cavity_mesh_area_A2":float(area[~keep].sum()),
        ...
        "containment_gate":"PASS_ALL_THREE_AXES_AGREE"}
    return keep,audit
