...
def binof(v):
 ...
 return GROUPS[4]
def main():
 src=R/'polymer_exposure_full_v2';out=R/'polymer_exposure_fine_strata_v2';out.mkdir();result=[]
 ...
 for ri,mat in enumerate(['PET','PA6','PA66']):
  ...
  audit=json.load(open(R/'polymer_exposure_audit_v1/summary.json'))
  refined=next(a for a in audit['materials'] if a['material']==mat)['refinement']
  numerically_uncertain=set()
  for q in refined:
   ii=next(i for i,r in enumerate(rows) if r['site_id']==q['site_id'])
   if bins[ii]!='CONNECTIVITY_UNRESOLVED' and bins[ii]!=binof(q['grid075_samples8192_exposure']):
    bins[ii]='NUMERICAL_BIN_UNCERTAIN';numerically_uncertain.add(q['site_id'])
  rec=dict(material=mat,n=len(rows),numerically_uncertain_sites=sorted(numerically_uncertain),counts={g:bins.count(g) for g in GROUPS+['CONNECTIVITY_UNRESOLVED','NUMERICAL_BIN_UNCERTAIN']},groups=[])
