...
def ses_grid(ce,h=.5):
 ...
 pad=int(np.ceil(20./min(g['step'][:2])))
 ext=np.pad(g['external'],((pad,pad),(pad,pad),(0,0)),mode='wrap')
 dist=distance_transform_edt(~ext,sampling=g['step'])
 phi=(dist[pad:-pad,pad:-pad,:]-1.4).astype(np.float32)
 del dist
 outside_distance=distance_transform_edt(ext,sampling=g['step'])[pad:-pad,pad:-pad,:]
 phi[g['external']]=-outside_distance[g['external']]-1.4
 del outside_distance,ext
 g['phi']=phi
 return g
...
def main():
 ...
 with (R/'RUNBOOK.md').open('a') as f:f.write('\nChemical surface/contact candidates: '+str(out)+'. Reproduce: CPU Python chemical_contact_surface_v3.py --output NEW_NAME. Dependencies isolated in deps_chem_contact_v1. Read definitions and per-core penetration audits; not a complete enzyme. Portable *_view.py files require PyMOL only.\n')
