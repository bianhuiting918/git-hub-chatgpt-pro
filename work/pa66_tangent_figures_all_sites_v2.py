#!/usr/bin/env python3
"""PA66-only postprocessing; immutable tangent scan; no catalyst scan."""
...
SRC=R/"tangent_probe_all_parent_20260909_v1"/"PA66"
OUT=R/"pa66_tangent_figures_all_sites_20260909_v2"
...
def cap_kind(mol,ci,li):
 ...
 if any(a.GetSymbol()=="C" and sum(v.GetAtomicNum()>1 for v in a.GetNeighbors())==1 for a in c.GetNeighbors()):return "ACYL_CAP"
...
def interp_ext(grid,p,env):
 ...
 v=map_coordinates(grid["external"].astype(np.float32,copy=False),c,order=1,mode="grid-wrap")
 v[outside]=1.
 return v
def read_mask(s):
...
def main():
 ...
 assert len(rows)==len(selected)==len(results)==6800
 byid={x["site_id"]:x for x in rows};assert len(byid)==6800
 assert set(byid)=={x["site_id"] for x in results}=={x["site_id"] for x in selected}
 ce=t.chem.ChemEnv("PA66");assert t.old.sha(R/"inputs_v2/PA66_parent_atoms.npz")==summary["source_atom_sha256"]
 mol=t.chem.Chem.SDMolSupplier(str(t.chem.sdf_for("PA66")),removeHs=False)[0];n=mol.GetNumAtoms()
 ...
 # Verify unchanged numerical results against all previous PA66 pilot/control records.
 comparisons=0
 for folder in ["tangent_probe_pilot_20260909_v1","tangent_probe_surface_controls_20260909_v1"]:
  p=R/folder/"PA66"/"summary.json"
  for s in json.load(open(p))["results"]:
   ...
   comparisons+=1
 t.old.emit("audit_pass",records=6800,prior_comparisons=comparisons,fit_sites=len(positive))
 ...
 np.savez_compressed(OUT/"all_site_section_fields.npz",fields_uint8=np.rint(fields*255).astype(np.uint8),axis_A=axis,site_ids=np.array([s["site_id"] for s in positive]))
 ...
 for b in range(4):
  ...
  for k in range(2):
   ...
   for site_index in ids:
    pr=fields[site_index,k].copy();pr[~circle]=np.nan
    if np.nanmin(pr)<.5<np.nanmax(pr):
     ax.contour(xx,zz,pr,levels=[.5],colors=[COLORS[b]],linewidths=.4,alpha=.025,zorder=1.5)
   group["individual_sections_requested"]=2*len(ids)
 ...
 fig.suptitle("PA66 backbone | carbonyl-centered interface by geometric exposure\nTop: x-z section; bottom: y-z section. Original slab top/bottom pooled.",fontsize=14)
 fig.supxlabel("Colored contours: blocked-space probability 0.25 / 0.50 / 0.75; pale: every included site.\n0.5 A probe-center exclusion envelope, not atomic SES. C center = origin; patch radius 15 A. Loose tails not independently removed.",fontsize=10)
 for ext in ["png","svg"]:fig.savefig(OUT/("pa66_exposure_interfaces."+ext),dpi=160)
 plt.close(fig)
 ...
 fig.suptitle("PA66 geometric accessibility | 6800 model amide bonds audited",fontsize=14)
 ...
 with (R/"RUN_LOG.jsonl").open("a") as f:f.write(json.dumps(dict(time=datetime.datetime.now(datetime.timezone.utc).isoformat(),event="pa66_tangent_figures_complete",source_sha256=t.old.sha(__file__),output=str(OUT),exit_status=0,seconds=audit["seconds"]))+"\n")
 with (R/"RUNBOOK.md").open("a") as f:f.write("\nPA66 tangent full-audit/figures: existing CPU Python pa66_tangent_figures_all_sites_v2.py; output "+str(OUT)+". Use NEW OUT path to rerun; no edits to active all-material scan. Definitions/denominators/loose-tail limitations in README and summary.\n")
 t.old.emit("pa66_figures_complete",output=str(OUT),fit_sites=len(positive),groups=groups,seconds=audit["seconds"])
