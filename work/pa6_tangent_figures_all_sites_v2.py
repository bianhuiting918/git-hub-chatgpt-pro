#!/usr/bin/env python3
"""PA6-only postprocessing; immutable tangent scan; no catalyst scan."""
...
SRC=R/"tangent_probe_all_parent_20260909_v1"/"PA6"
OUT=R/"pa6_tangent_figures_all_sites_20260909_v2"
...
def main():
 ...
 ce=t.chem.ChemEnv("PA6");assert t.old.sha(R/"inputs_v2/PA6_parent_atoms.npz")==summary["source_atom_sha256"]
 mol=t.chem.Chem.SDMolSupplier(str(t.chem.sdf_for("PA6")),removeHs=False)[0];n=mol.GetNumAtoms()
 ...
 # Verify unchanged numerical results against all previous PA6 pilot/control records.
 comparisons=0
 for folder in ["tangent_probe_pilot_20260909_v1","tangent_probe_surface_controls_20260909_v1"]:
  p=R/folder/"PA6"/"summary.json"
 ...
 for ext in ["png","svg"]:fig.savefig(OUT/("pa6_exposure_interfaces."+ext),dpi=160)
 plt.close(fig)
 ...
 fig.suptitle("PA6 geometric accessibility | 6800 model amide bonds audited",fontsize=14)
 ...
 report=["# PA6: full-catalogue geometric exposure and aligned interface sections","","Numerical audit complete. Figures are descriptive material geometry, not enzyme activity.","",f"All {len(results)} model bonds audited; {audit['accessible_model_bonds']} have sampled external access. Main fits use {len(positive)} backbone sites. Caps are separate; loose-tail classification remains NOT_EVALUATED.","","## Figures","","![Interfaces](pa6_exposure_interfaces.png)","","![Statistics](pa6_exposure_statistics.png)","","## Groups","","| S_log group | Sites | Median sampled Rmax (A) | Rmax >=4 A | Representative |","|---|---:|---:|---:|---|"]
 ...
 with (R/"RUN_LOG.jsonl").open("a") as f:f.write(json.dumps(dict(time=datetime.datetime.now(datetime.timezone.utc).isoformat(),event="pa6_tangent_figures_complete",source_sha256=t.old.sha(__file__),output=str(OUT),exit_status=0,seconds=audit["seconds"]))+"\n")
 with (R/"RUNBOOK.md").open("a") as f:f.write("\nPA6 tangent full-audit/figures: existing CPU Python pa6_tangent_figures_all_sites_v2.py; output "+str(OUT)+". Use NEW OUT path to rerun; no edits to active all-material scan. Definitions/denominators/loose-tail limitations in README and summary.\n")
 t.old.emit("pa6_figures_complete",output=str(OUT),fit_sites=len(positive),groups=groups,seconds=audit["seconds"])
