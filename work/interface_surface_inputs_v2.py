...
if __name__=='__main__':
 O=R/'inputs_v2';O.mkdir(exist_ok=False);report=[];rc=1
 try:
  for mat,dp in [('PET',10),('PA6',16),('PA66',8)]:
   ...
   if mat=='PET':
    lines=pdb.read_text().splitlines();cr=next(l for l in lines if l.startswith('CRYST1'))
    box=np.array([float(cr[i:j]) for i,j in [(6,15),(15,24),(24,33)]])
    atoms=[dict(line=l,serial=l[6:11],element=l[76:78].strip(),xyz=np.array([float(l[i:i+8]) for i in (30,38,46)])) for l in lines if l.startswith(('ATOM  ','HETATM'))]
   else:
