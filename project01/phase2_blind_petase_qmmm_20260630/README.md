...
blind_work/00_run_status/petase_qmmm_reproduction_guide_zh_20260706.md
```

These files are the main handoff records for reproducing the blind PETase
...
The Chinese guide explains which result files to inspect, which columns matter,
and how each decision should be interpreted.
...
tests/test_generate_stage4_amber_qmmm_inputs.py
tests/test_prepare_stage4_amber_topology_inputs.py
tests/test_map_stage4_amber_qmmm_masks_to_topology.py
tests/test_prepare_stage4_amber_cleanup_inputs.py
```
...
2. Prepare Amber/Sander DFTB3 QM/MM minimization and 200 ps equilibration inputs; prepare AmberTools ligand/protein topology inputs; remap QM atom masks onto tleap atom indices; run staged MM/QM-MM cleanup minimization; then run relaxed scans or TS-search seeds only from this blind workflow.
