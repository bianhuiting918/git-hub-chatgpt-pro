...
RUN_ID="${RUN_ID:-ca_rfd_baker_theozyme_diffusion_batch_publicckpt}"
NUM_DESIGNS="${NUM_DESIGNS:-5}"
DESIGN_STARTNUM="${DESIGN_STARTNUM:-1}"
...
if [ ! -x "$PY" ]; then
  echo "Missing CA_RFDiffusion python: $PY" >&2
  exit 2
fi
if [ ! -s "$CKPT" ]; then
  echo "Missing public CA_RFDiffusion checkpoint: $CKPT" >&2
  exit 2
fi
if [ ! -f "$CA_RFD/rf_diffusion/run_inference.py" ]; then
  echo "Missing CA_RFDiffusion run_inference.py under: $CA_RFD" >&2
  exit 2
fi
  ...
  cat > "$STATUS" <<JSON
{
  ...
  "run_id": "$RUN_ID",
  "route_label": "baker_theozyme_new_backbone_public_ca_rfdiffusion",
  "root": "$ROOT",
  "input": "$INPUT",
  "ckpt": "$CKPT",
  ...
  "next_action": "Wait until GPU compute processes clear, or intentionally set ALLOW_SHARED_GPU=1 / FORCE=1."
}
JSON
  echo "$STATUS"
  exit 0
...
cat > "$MANIFEST" <<TSV
run_id	route_label	input_pdb	ckpt	ligand	contig	num_designs	design_startnum	diffuser_T	status	pid	log	output_prefix
