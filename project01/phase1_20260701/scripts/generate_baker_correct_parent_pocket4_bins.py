...
OUT = ROOT / "sequence_design/baker_correct_parent_pocket4_ligandmpnn_bins"
MANIFEST = ROOT / "manifests/baker_correct_parent_pocket4_ligandmpnn_bins_selected.tsv"
SUMMARY = ROOT / "manifests/baker_correct_parent_pocket4_ligandmpnn_bins_summary.json"
PRED_DIR = ROOT / "structure_prediction/esmfold_baker_correct_parent_pocket4_bins"

CATALYTIC_AND_OXY = {15, 16, 55, 73, 149}
POCKET_HEAVY_CUTOFF_A = 4.0
SELECT_PER_BIN = 40
BINS = {
    "90": {"target_mutations": 16, "seed": 95301, "temperature": 0.10, "batch_size": 40, "batches": 3},
    "80": {"target_mutations": 32, "seed": 85301, "temperature": 0.12, "batch_size": 40, "batches": 3},
    "70": {"target_mutations": 48, "seed": 75301, "temperature": 0.15, "batch_size": 40, "batches": 3},
    "60": {"target_mutations": 64, "seed": 65301, "temperature": 0.18, "batch_size": 40, "batches": 3},
    "50": {"target_mutations": 80, "seed": 55301, "temperature": 0.22, "batch_size": 40, "batches": 3},
}
...
def main() -> int:
    ...
    for bin_id, spec in BINS.items():
        ...
        for rank, row in enumerate(passed, 1):
            sample_id = f"baker_correct_parent_pocket4_bin{bin_id}_rank{rank}_rec{row['record_index']}"
            selected_all.append({
                "sample_id": sample_id,
                "bin": row["bin"],
