#!/usr/bin/env python3
"""Prepare the frozen NylC A1 representative frame for a expanded DFTB3 numerical preflight."""
import argparse
import hashlib
import json
import os
...
def derive_qm_contract(structure):
    ...
    if len(boundary) != EXPECTED_LINK_ATOMS:
        raise ValueError(f"expected three QM/MM boundary bonds, observed {boundary}")
    observed_boundaries = {(item["qm_atom"], item["mm_atom"]) for item in boundary}
    expected_boundaries = {(8962, 8964), (9567, 9565), (9587, 9585)}
...
def main():
    ...
    (output / "01_qmmm_one_step.in").write_text(
        qmmm_input("NylC A1 expanded Asp306/Asp308 DFTB3 one-step numerical preflight", maxcyc=1, qmmask=qmmask),
        encoding="utf-8",
    )
    (output / "02_qmmm_20_step.in").write_text(
        qmmm_input("NylC A1 expanded Asp306/Asp308 DFTB3 twenty-step numerical preflight", maxcyc=20, qmmask=qmmask),
        encoding="utf-8",
    )
    audit = {
        "schema_version": 1,
        "status": "READY_A1_EXPANDED_DFTB3_NUMERICAL_PREFLIGHT",
        ...
        "qmmask": qmmask,
        "qm_region_scope": "expanded Thr267 plus Asp306/ASH and Asp308/ASP sidechains plus complete L2 numerical preflight",
        "production_qm_region_gate": (
            "Asp306 and Asp308 are included; before production Step1 TS/PMF, test "
            "network sensitivity including Tyr146, Lys189, Asn219 and relevant water."
        ),
        "interpretation": (
