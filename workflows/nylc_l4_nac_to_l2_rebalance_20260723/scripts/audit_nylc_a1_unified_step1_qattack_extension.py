#!/usr/bin/env python3
"""Independently audit the pinned A1 Step1 q_attack extension window."""
import argparse
import json
import math
import pathlib
...
def inspect_window(prmtop, directory, target):
    ...
    plausible = len(values) == 7 and all(
...
def main():
    ...
    if manifest.get("status") != "READY_A1_UNIFIED_STEP1_QATTACK_EXTENSION":
        raise SystemExit("extension manifest is not READY")
    if len(manifest.get("windows", [])) != 1:
        raise SystemExit("extension manifest must contain exactly one window")

    start_geometry = geometry(args.source_prmtop, args.start_restart)
    item = manifest["windows"][0]
    window = inspect_window(args.source_prmtop, scan_root / item["name"], item["target_A"])
    technical = window["technical_pass"]
    endpoint = window["geometry"] if technical else {}
    bracket = technical and (
    ...
    result = {
        ...
        "status": (
            ...
            if technical else "FAIL_TECHNICAL_A1_UNIFIED_STEP1_QATTACK_EXTENSION"
        ),
        ...
        "window": window,
        "interpretation": (
            "Minimal constrained attack-bracket extension. A passing seed is not a TS, "
