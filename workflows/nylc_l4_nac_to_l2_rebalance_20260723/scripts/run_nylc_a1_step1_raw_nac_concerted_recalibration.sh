...
: "${A1_CONCERTED_RECAL_CODE_ROOT:?set immutable workflow root}"
: "${A1_CONCERTED_RECAL_GITHUB_COMMIT:?set immutable GitHub commit}"

TASK_ROOT=/work/home/acshdt1dks/nylon_pa66_scnet_20260708/l4_nac_to_l2_rebalance_20260723
CODE_ROOT="$A1_CONCERTED_RECAL_CODE_ROOT"
COMMIT="$A1_CONCERTED_RECAL_GITHUB_COMMIT"
...
OUT_ROOT="$TASK_ROOT/a1_activated_nac_20260726/qmmm/a1_step1_raw_nac_concerted_recalibration"
OUT="$OUT_ROOT/attempt_${ARRAY_JOB}_${INDEX}"
SCRATCH="${SLURM_TMPDIR:-/tmp}/nylc_a1_concerted_recal_${ARRAY_JOB}_${INDEX}"
DRIVER="$CODE_ROOT/scripts/prepare_audit_nylc_a1_step1_raw_nac_concerted_recalibration.py"
...
append_history() {
    ...
    (
        flock -x 9
        printf '%s\t%s\t%s\t%s\t%s\t%s\n' "$now" "nylc_a1_step1_raw_nac_concerted_recalibration" "run_nylc_a1_step1_raw_nac_concerted_recalibration.sh" "$state" "$COMMIT" "$detail" >> "$TASK_ROOT/run_history.tsv"
        printf '{"time":"%s","event":"nylc_a1_step1_raw_nac_concerted_recalibration","state":"%s","git_commit":"%s","detail":"%s","output":"%s"}\n' "$now" "$state" "$COMMIT" "$detail" "$OUT" >> "$TASK_ROOT/run_history.jsonl"
...
test ! -e "$OUT"
test ! -e "$SCRATCH"
append_history STARTED "job=$ARRAY_JOB;task=$INDEX;source_calibration=62471131;concerted_local_recalibration=true"
