#!/usr/bin/env bash
# Amber18 A1 DFTB3 numerical preflight; no MM or frame selection occurs here.
set -euo pipefail
...
OUT="$TASK_ROOT/a1_activated_nac_20260726/qmmm/a1_dftb3_preflight/attempt_$ATTEMPT"
GITHUB_COMMIT="${A1_GITHUB_COMMIT:-unknown}"
[[ ! -e "$OUT" ]] || { printf 'refusing to overwrite %s\n' "$OUT" >&2; exit 2; }
mkdir -p "$OUT"
EVENT=nylc_a1_dftb3_numerical_preflight
COMMAND=run_nylc_a1_dftb3_preflight.sh
STATE=STARTED
CURRENT=initialization
DETAIL="source=$REP;scope=numerical_preflight_not_TS_PMF_barrier;output=$OUT"
append_history() {
    local now
...
import json,sys
if json.load(open(sys.argv[1],encoding="utf-8")).get("status")!="PASS_A1_DFTB3_NUMERICAL_PREFLIGHT":
...
DETAIL="status=PASS_A1_DFTB3_NUMERICAL_PREFLIGHT;scientific_status=NOT_EVALUATED_TS_PMF_BARRIER;output=$OUT/PASS.json"
append_history
trap - EXIT
