#!/usr/bin/env bash
set -euo pipefail
: "${A1_STEP2_WATERNET_CODE_ROOT:?set immutable code root}"
: "${A1_STEP2_WATERNET_GITHUB_COMMIT:?set immutable GitHub commit}"
TASK_ROOT=/work/home/acshdt1dks/nylon_pa66_scnet_20260708/l4_nac_to_l2_rebalance_20260723
CODE_ROOT="$A1_STEP2_WATERNET_CODE_ROOT"
GITHUB_COMMIT="$A1_STEP2_WATERNET_GITHUB_COMMIT"
...
OUT_ROOT="$TASK_ROOT/a1_activated_nac_20260726/qmmm/a1_step2_water_network_sampling"
OUT="$OUT_ROOT/attempt_${ARRAY_JOB}_${INDEX}"
SCRATCH_ROOT="${SLURM_TMPDIR:-/tmp}/nylc_a1_step2_water_network_${ARRAY_JOB}_${INDEX}"
...
append_history() {
    ...
    (
        flock -x 9
        printf '%s\t%s\t%s\t%s\t%s\t%s\n' "$now" "nylc_a1_step2_water_network_sampling" "run_nylc_a1_step2_water_network_sampling.sh" "$state" "$GITHUB_COMMIT" "$detail" >> "$TASK_ROOT/run_history.tsv"
        printf '{"time":"%s","event":"nylc_a1_step2_water_network_sampling","state":"%s","git_commit":"%s","detail":"%s","output":"%s"}\n' "$now" "$state" "$GITHUB_COMMIT" "$detail" "$OUT" >> "$TASK_ROOT/run_history.jsonl"
