#!/usr/bin/env bash
# Amber18 A1 expanded Asp306/Asp308 DFTB3 numerical preflight; no MM or frame selection occurs here.
set -euo pipefail
TASK_ROOT=/work/home/acshdt1dks/nylon_pa66_scnet_20260708/l4_nac_to_l2_rebalance_20260723
CODE_ROOT="${A1_EXPANDED_DFTB3_CODE_ROOT:?set immutable code snapshot}"
...
ATTEMPT="${A1_EXPANDED_DFTB3_ATTEMPT:-${SLURM_JOB_ID:-manual_$(date -u '+%Y%m%dT%H%M%SZ')}}"
OUT="$TASK_ROOT/a1_activated_nac_20260726/qmmm/a1_expanded_dftb3_preflight/attempt_$ATTEMPT"
GITHUB_COMMIT="${A1_EXPANDED_GITHUB_COMMIT:-unknown}"
[[ ! -e "$OUT" ]] || { printf 'refusing to overwrite %s\n' "$OUT" >&2; exit 2; }
mkdir -p "$OUT"
EVENT=nylc_a1_expanded_dftb3_numerical_preflight
COMMAND=run_nylc_a1_expanded_dftb3_preflight.sh
STATE=STARTED
CURRENT=initialization
DETAIL="source=$REP;scope=expanded_Asp306_Asp308_numerical_preflight_not_TS_PMF_barrier;output=$OUT"
append_history() {
    local now
...
pathlib.Path(path).write_text(json.dumps({"schema_version":1,"status":"NOT_EVALUATED_A1_EXPANDED_DFTB3_TECHNICAL_FAILURE","scientific_status":"NOT_EVALUATED","failed_stage":stage,"exit_code":int(code),"promoted":False},indent=2,sort_keys=True)+"\n",encoding="utf-8")
PY
...
CURRENT=preparation
"$PY" "$CODE_ROOT/scripts/prepare_nylc_a1_expanded_dftb3_preflight.py" \
...
CURRENT=one_step
mpirun -np "${SLURM_NTASKS:-1}" sander.MPI -O -i "$OUT/prepared/01_qmmm_one_step.in" -o "$OUT/prepared/01_qmmm_one_step.out" -p "$OUT/prepared/system.prmtop" -c "$OUT/prepared/representative_354ps.rst7" -r "$OUT/prepared/01_qmmm_one_step.rst7" -inf "$OUT/prepared/01_qmmm_one_step.mdinfo"
CURRENT=twenty_step
mpirun -np "${SLURM_NTASKS:-1}" sander.MPI -O -i "$OUT/prepared/02_qmmm_20_step.in" -o "$OUT/prepared/02_qmmm_20_step.out" -p "$OUT/prepared/system.prmtop" -c "$OUT/prepared/01_qmmm_one_step.rst7" -r "$OUT/prepared/02_qmmm_20_step.rst7" -inf "$OUT/prepared/02_qmmm_20_step.mdinfo"
CURRENT=numerical_audit
set +e
"$PY" "$CODE_ROOT/scripts/audit_nylc_a1_expanded_dftb3_smoke.py" --one-step "$OUT/prepared/01_qmmm_one_step.out" --segment "$OUT/prepared/02_qmmm_20_step.out" --audit "$OUT/prepared/qmmm_preflight_audit.json" --output "$OUT/A1_DFTB3_PREFLIGHT_RESULT.json"
audit_rc=$?
set -e
[[ "$audit_rc" -eq 0 ]]
"$PY" - "$OUT/A1_DFTB3_PREFLIGHT_RESULT.json" <<'PY'
import json,sys
if json.load(open(sys.argv[1],encoding="utf-8")).get("status")!="PASS_A1_EXPANDED_DFTB3_NUMERICAL_PREFLIGHT":
...
DETAIL="status=PASS_A1_EXPANDED_DFTB3_NUMERICAL_PREFLIGHT;scientific_status=NOT_EVALUATED_TS_PMF_BARRIER;output=$OUT/PASS.json"
append_history
trap - EXIT
