...
: "${A1_RAW_NAC_PREORG_CODE_ROOT:?set immutable code root}"
: "${A1_RAW_NAC_PREORG_GITHUB_COMMIT:?set immutable GitHub commit}"

TASK_ROOT=/work/home/acshdt1dks/nylon_pa66_scnet_20260708/l4_nac_to_l2_rebalance_20260723
CODE_ROOT="$A1_RAW_NAC_PREORG_CODE_ROOT"
COMMIT="$A1_RAW_NAC_PREORG_GITHUB_COMMIT"
...
OUT_ROOT="$TASK_ROOT/a1_activated_nac_20260726/qmmm/a1_step1_raw_nac_preorganized_calibration"
OUT="$OUT_ROOT/attempt_${ARRAY_JOB}_${INDEX}"
SCRATCH="${SLURM_TMPDIR:-/tmp}/nylc_a1_raw_nac_preorg_${ARRAY_JOB}_${INDEX}"
DRIVER="$CODE_ROOT/scripts/prepare_audit_nylc_a1_step1_raw_nac_preorganized_calibration.py"
...
append_history() {
    ...
    (
        flock -x 9
        printf '%s\t%s\t%s\t%s\t%s\t%s\n' "$now" "nylc_a1_step1_raw_nac_preorganized_calibration" "run_nylc_a1_step1_raw_nac_preorganized_calibration.sh" "$state" "$COMMIT" "$detail" >> "$TASK_ROOT/run_history.tsv"
        printf '{"time":"%s","event":"nylc_a1_step1_raw_nac_preorganized_calibration","state":"%s","git_commit":"%s","detail":"%s","output":"%s"}\n' "$now" "$state" "$COMMIT" "$detail" "$OUT" >> "$TASK_ROOT/run_history.jsonl"
...
test ! -e "$OUT"
test ! -e "$SCRATCH"
append_history STARTED "job=$ARRAY_JOB;task=$INDEX;matrix=2seeds_x_3attack_x_3carbonyl;source=raw_nac"
...
if [[ "$STATUS" == NOT_EVALUATED_TECHNICAL_* ]]; then exit 1; fi
SUCCESS=1
