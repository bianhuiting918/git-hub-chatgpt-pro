#!/usr/bin/env python3
"""Audit ASH306 core/network one-step DFTB3 numerical smokes."""
import argparse
import json
import pathlib
import re
...
def main():
    ...
    passed = (
        region_audit.get("status") == "READY_FOR_ASH306_QM_REGION_SMOKE"
        and region_audit.get("protonation_microstate") == "Asp306H_Asp308-"
        and all(item["passed"] for item in results.values())
    )
    result = {
        "schema_version": 1,
        "candidate_id": "nylc_C18_trueT267_freeGS",
        "status": "PASS_ASH306_QM_REGION_SMOKE" if passed else "FAIL_ASH306_QM_REGION_SMOKE",
        "regions": results,
        "protonation_microstate": region_audit.get("protonation_microstate"),
        "region_definition": region_audit.get("regions"),
        "interpretation": "Numerical one-step result only; not a TS, RC, PMF, or barrier.",
        "next": "If PASS, define Step1 proton-transfer and nucleophilic-attack coordinate candidates without ranking microstates by raw absolute energy.",
    }
